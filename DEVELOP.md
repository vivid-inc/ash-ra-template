...
- Allow the batch specification `templates` argument to also accept quoted varnames to an IFn, in addition to stringified file paths. Functions will be called with the batch as their first argument and are expected to return a seq of render jobs.
- CLI option for `render` to either fail command by first attempting the entire batch then reporting exit code `--fail-collect` (default, consistent with `watch`), or at first render error `--fail-fast`.
...
- Explore something similar to a `(defmethod)` mechanism for adding options to `(vivid.art/render)`, and try it with `:classpath` and `:repositories` options in `art-cli`.
...
- The purpose of ART is multifold: An ideal substrate for building a custom templating solution such as the constrained Jinja or something more flexible, and as a fully-featured templating system in its own right.
- babashka, jank-lang.
